...
DECLARE
    ...
    v_proximo_mes_inicio date := (date_trunc('month', current_date) + interval '1 month')::date;
BEGIN
    SELECT 
        COALESCE(SUM(GREATEST(0, valor_total - COALESCE(valor_pago, 0))), 0) AS total_liquidar,
        COALESCE(SUM(CASE WHEN data_vencimento = v_hoje THEN GREATEST(0, valor_total - COALESCE(valor_pago, 0)) ELSE 0 END), 0) AS vencendo_hoje,
        COALESCE(SUM(CASE WHEN data_vencimento < v_hoje THEN GREATEST(0, valor_total - COALESCE(valor_pago, 0)) ELSE 0 END), 0) AS total_atrasado
    INTO
        v_total_liquidar,
        v_vencendo_hoje,
        v_total_atrasado
