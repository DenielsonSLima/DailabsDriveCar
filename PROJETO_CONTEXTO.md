...
- **KPIs em Branco (Skeleton Infinito)**: NUNCA usar `!== undefined` para decidir se mostra valor ou skeleton em componentes de KPI. O padrão correto é: usar `?? 0` para fallback e prop `loading` para skeleton. Além disso, SEMPRE usar `Promise.allSettled` (não `Promise.all`) ao carregar dados e KPIs juntos, para que um erro na RPC não derrube a lista inteira. Sempre fazer `setKpis(null)` no catch.
- **Relacionamentos Inexistentes em Queries (Erros PostgREST)**: NUNCA incluir a coluna `comissao_percentual` na query de corretores (`cad_corretores`), pois essa coluna não existe fisicamente no banco de dados (o frontend realiza o cálculo de comissões aplicando fallback dinâmico de 3%). NUNCA tentar realizar JOIN de `fin_titulos` com `fin_contas_bancarias` na query de títulos pendentes (títulos pendentes/em aberto de contas a pagar/receber não possuem conta bancária associada; a conta só é definida e gravada na tabela `fin_transacoes` no momento em que o título é liquidado).
