-- Migration: Regime de Competência para Outros Créditos no Lucro Mensal
-- Date: 2026-06-09
-- Objetivo: Considerar os lançamentos de "Outros Créditos" no Lucro Mensal do mês de lançamento (data_vencimento),
-- mesmo que ainda não tenham sido recebidos/pagos (Regime de Competência).
...
BEGIN
    ...
    -- 9.1. Outros Créditos Lançados (Regime de Competência baseada em data_vencimento)
    SELECT COALESCE(SUM(valor_total), 0) INTO v_total_outros_creditos
    FROM public.fin_titulos
    WHERE organization_id = v_org_id 
      AND tipo = 'RECEBER' 
      AND origem_tipo = 'OUTRO_CREDITO'
      AND status != 'CANCELADO'
    ...
    RETURN jsonb_build_object(
        ...
        -- O Lucro Mensal agora soma os descontos obtidos e subtrai descontos concedidos, e soma outros créditos lançados no período
