-- Migration: Correção de Descontos e Acréscimos nas Métricas de Caixa e Lucro
-- Data: 2026-06-03
-- Objetivo: Garantir que descontos obtidos e concedidos sejam computados corretamente no Lucro Mensal,
-- e não poluam os totais de entradas e saídas de caixa (já que são transações sem movimentação financeira física).
...
BEGIN
    ...
    RETURN jsonb_build_object(
        ...
        -- O Lucro Mensal agora soma os descontos obtidos e subtrai descontos concedidos
        'lucro_mensal', (v_total_vendas_recebido - v_total_custo_vendas + v_lucro_adicionais + v_total_descontos_obtidos) - (v_total_despesas_fixas + v_total_despesas_variaveis + v_total_outros_debitos + v_total_descontos_concedidos),
        'lucro_gerado', (v_total_vendas_recebido - v_total_custo_vendas + v_lucro_adicionais + v_total_descontos_obtidos) - (v_total_despesas_fixas + v_total_despesas_variaveis + v_total_outros_debitos + v_total_descontos_concedidos),
        'lucro_realizado', v_total_entradas - v_total_saidas,
        'lucro_pendente', v_total_recebiveis,
        'margem_lucro', CASE WHEN (v_total_vendas_recebido + v_lucro_adicionais) > 0 THEN (((v_total_vendas_recebido - v_total_custo_vendas + v_lucro_adicionais + v_total_descontos_obtidos) - (v_total_despesas_fixas + v_total_despesas_variaveis + v_total_outros_debitos + v_total_descontos_concedidos)) / (v_total_vendas_recebido + v_lucro_adicionais)) * 100 ELSE 0 END
    );
END;
$function$;
