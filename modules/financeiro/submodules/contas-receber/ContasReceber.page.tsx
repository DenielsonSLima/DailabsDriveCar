...
const ContasReceberPage: React.FC = () => {
  ...
  // Cada título mantém seu vencimento e seu próprio saldo para a baixa.
  // Agrupar após paginar ocultava parcelas e misturava valores de títulos distintos.
  const titulos = useMemo(() => data?.data || [], [data]);
  ...
  const processedData = useMemo(() => {
    ...
    return titulos.reduce((acc: any, t) => {
      let key = 'DIVERSOS';
      if (groupBy === 'mes') {
        const d = parseDateOnly(t.data_vencimento);
