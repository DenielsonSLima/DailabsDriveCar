...
import { ITituloDebito, DebitosTab, IDebitoFiltros } from './outros-debitos.types';

const TABLE = 'fin_titulos';

export const OutrosDebitosService = {
  async getAll(tab: DebitosTab, filtros: IDebitoFiltros) {
    let query = supabase
      .from(TABLE)
    ...
    return { data: data as ITituloDebito[], count: count || 0 };
  },
  ...
  async save(payload: {
    ...
    const { data, error } = await supabase.rpc('lancar_debito', {
  ...
  async getFormasPagamento() {
    const { data } = await supabase
      ...
      .select('*')
      .eq('ativo', true)
      .in('tipo_movimentacao', ['PAGAMENTO', 'AMBOS'])
      .neq('destino_lancamento', 'CONSIGNACAO')
      .order('nome');
    return data;
  },

  async baixarDebito(tituloId: string, payload: {
    valor: number;
  ...
  async getKpis() {
    const { data, error } = await supabase.rpc('rpc_kpi_outros_debitos');

    if (error) {
      console.error('Erro ao buscar KPIs de outros débitos:', error);
      throw error;
    }
  ...
  subscribe(callback: () => void) {
    return supabase
      .channel('fin_outros_debitos_sync')
