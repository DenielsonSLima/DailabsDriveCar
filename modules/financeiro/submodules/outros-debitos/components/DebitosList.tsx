...
const DebitosList: React.FC<Props> = ({ items, loading, onPagar, onEdit, onDelete, onBaixa, viewMode = 'list', pagination }) => {
  ...
  if (loading) return (
    <div className="py-32 flex flex-col items-center justify-center space-y-4">
      ...
      <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">Sincronizando débitos...</p>
    </div>
  );

  const renderCards = (rows: ITituloDebito[]) => (
  ...
  const renderTable = (rows: ITituloDebito[]) => (
      ...
      <table className="w-full text-left">
        <thead className="bg-slate-50 border-b border-slate-100">
          <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
            ...
            <th className="px-8 py-5">Descrição / Motivo</th>
            <th className="px-8 py-5 text-right">Valor Total</th>
            <th className="px-8 py-5 text-right text-emerald-600">Pago</th>
        ...
        <tbody className="divide-y divide-slate-50">
          {rows.map((t) => (
            ...
            >
              ...
              <td className="px-8 py-6">
                <span className={`px-2 py-1 rounded text-[8px] font-black uppercase tracking-widest border ${t.status === 'PAGO' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'
                  }`}>
                  {t.status}
                </span>
              </td>
  ...
  const content = () => {
    ...
    if (list.length === 0) return <div className="py-32 text-center text-slate-400 font-bold uppercase text-[10px] tracking-widest border-2 border-dashed border-slate-100 rounded-3xl mx-8">Nenhum débito extraordinário encontrado</div>;
