...
const TransmissaoPage: React.FC = () => {
  ...
  const loadData = async (silent = false) => {
    ...
    try {
      const data = await TransmissaoService.getAll(displayStatus === 'active');
      setItems(data);
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmInactivate = async () => {
    ...
    try {
      await TransmissaoService.remove(inactivateId);
      showToast('success', 'Transmissão inativada com sucesso.');
      loadData(true);
  ...
  const handleReactivate = async (id: string) => {
    try {
      await TransmissaoService.reactivate(id);
      showToast('success', 'Transmissão reativada com sucesso!');
      loadData(true);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      <ConfirmModal 
        ...
        title="Inativar Transmissão?"
        message={`Deseja inativar a transmissão "${items.find(i => i.id === inactivateId)?.nome}"? Ela não aparecerá mais para novos cadastros de estoque.`}
