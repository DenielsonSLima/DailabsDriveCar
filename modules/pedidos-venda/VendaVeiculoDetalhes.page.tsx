...
const VendaVeiculoDetalhesPage: React.FC = () => {
  ...
  async function loadData() {
    ...
    try {
      const [vData, carData, opData, coresData, pData] = await Promise.all([
        ...
        CoresService.getAll(),
        id ? PedidosVendaService.getById(id) : Promise.resolve(null)
      ]);
  ...
  const handleAddExpense = async (expenses: Partial<IVeiculoDespesa>[]) => {
    ...
    if (pedido?.status === 'RASCUNHO') {
      toast.error('Este pedido ainda é um rascunho. Confirme a venda para liberar o lançamento de despesas financeiras.');
      return;
    }
        ...
        <div className="mb-8 p-6 bg-amber-50 border-2 border-amber-200 rounded-[2rem] flex items-center space-x-6 animate-in fade-in slide-in-from-top duration-500">
          ...
          <div>
            ...
            <p className="text-amber-700 text-sm font-medium leading-relaxed">
              Esta venda ainda é um <span className="font-bold underline">Rascunho não confirmado</span>. 
              Para realizar lançamentos financeiros definitivos neste veículo, você deve primeiro efetivar a venda na tela anterior.
            </p>
          </div>
        </div>
      )}
