...
export const CaixaDashboardSchema = z.object({
  ...
  total_outros_debitos: z.number().nullish().transform(v => v ?? 0),
  ...
  lucro_gerado: z.number().nullish().transform(v => v ?? 0),
  lucro_realizado: z.number().nullish().transform(v => v ?? 0),
  lucro_pendente: z.number().nullish().transform(v => v ?? 0),
